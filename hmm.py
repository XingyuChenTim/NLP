...
class HMM:
    ...
    def fit(self, state_ids: List[List[int]], observation_ids: List[List[int]]):
        ...
        for s_ids, o_ids in zip(state_ids, observation_ids):
            ...
            self.pi[s_ids[0]] += 1
            for j, k in zip(range(0, len(s_ids)), range(1, len(s_ids))):
                self.A[s_ids[j], s_ids[k]] += 1
            for j, k in zip(range(0, len(s_ids)), range(0, len(s_ids))):
                self.B[s_ids[j], o_ids[k]] += 1
    ...
    def decode(self, observation_ids: List[List[int]]) -> List[List[int]]:
        ...
        for obs_ids in observation_ids:
            ...
            for i in range(self.n):
                viterbi[i][0] = np.exp(self.pi[i] + self.B[i][obs_ids[0]])
                back_pointer[i][0] = 0

            for t in range(1, T):
                for i in range(self.n):
                    viterbi[i][t] = max(viterbi[:, t-1] * np.exp( self.A[:, i] + self.B[i][obs_ids[t]]))
                    back_pointer[i][t] = np.argmax(viterbi[:, t-1] * np.exp(self.A[:, i] + self.B[i][obs_ids[t]]))

            bestpathpointer = int(np.argmax(viterbi[:, -1]))
            S[T-1] = bestpathpointer

            for i in range(T - 2, -1, -1):
                S[i] = int(back_pointer[int(S[i+1])][i+1])

            all_predictions.append(S)
